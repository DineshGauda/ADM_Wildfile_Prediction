...
wildfireData=read.csv("wildfire_data.csv",stringsAsFactors = F)
str(wildfireData)
...
resultUnique <- resultUnique[!resultUnique$V2=="V1",]
resultUnique$V2 <- as.Date(resultUnique$V2, "%Y-%m-%d")
resultUnique <- resultUnique[resultUnique$V2 < "2011-01-01",]

write.csv(resultUnique, file = "resultUnique2.csv",row.names=FALSE)

dim(resultUnique)
write.csv(myfiles,file = "mainFile.csv",row.names=FALSE)
